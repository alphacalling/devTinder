...
export default function Chat() {
  ...
  const getPhoto = (photoUrl) => {
    ...
    return photoUrl || "https://via.placeholder.com/40";
  };
  ...
  const primaryPhoto = getPhoto(selectedUser?.photoUrl);
  const myPhoto = getPhoto(currentUser?.photoUrl);
      ...
      <div className="w-80 shrink-0 border-r bg-white flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            conversations.map((conv) => {
              const u = conv.user;
              const photo = getPhoto(u.photoUrl);
              const isActive = selectedUser?._id === u._id;
              return (
                <button
                  key={u._id}
                  ...
                  className={`w-full flex items-center gap-3 p-3 text-left border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                    isActive ? "bg-pink-50" : ""
                  }`}
                >
                  <img
          ...
          <>
            ...
            <div className="p-3 border-t flex gap-2 bg-white">
              ...
              <button
                ...
                className="px-5 py-2 bg-pink-500 hover:bg-pink-600 disabled:bg-pink-300 disabled:cursor-not-allowed text-white rounded-xl font-medium transition-colors"
              >
                Send
              </button>
            </div>
